...
module.exports = function(pool, settings, callback) {
  var queries = [];
  var data = generate.apply(null, settings);

  // Used by pg-live-query.
  queries.push('DROP SEQUENCE IF EXISTS __rev___sequence CASCADE');

  for(var table in data) {
    queries.push('DROP TABLE IF EXISTS ' + table + ' CASCADE');
  ...
  querySequence(pool, queries, callback);
};
